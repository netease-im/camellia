...
public class PrometheusMetrics {
    ...
    public static String metrics() {
        ...
        builder.append(prefix).append(String.format("thread %d\n", hotKeyServerStats.getQueueStats().getQueueNum()));
        ...
        builder.append(prefix).append(String.format("client_connect %d\n", connectCount));
        ...
        for (QueueStats.Stats stats : statsList) {
            builder.append(prefix).append(String.format("queue{name=\"%s\", type=\"pending\"} %d\n", stats.getId(), stats.getPendingSize()));
            builder.append(prefix).append(String.format("queue{name=\"%s\", type=\"discard\"} %d\n", stats.getId(), stats.getDiscardCount()));
        }
        ...
        for (TrafficStats.Stats stats : trafficStatsStatsList) {
            builder.append(prefix).append(String.format("qps_detail{namespace=\"%s\", type=\"%s\"} %d\n",
        ...
        for (HotKeyInfo hotKeyInfo : hotKeyInfoList) {
            ...
            builder.append(prefix).append(String.format("hot_key{namespace=\"%s\", key=\"%s\", action=\"%s\", rule=\"%s\", source=\"%s\"} %d\n",
