 ...
 */
public class SidCarFirstProxySelector implements IProxySelector {

    private static final Logger logger = LoggerFactory.getLogger(SidCarFirstProxySelector.class);

    private final Object lock = new Object();

    private final String localhost;
    private Proxy sidCarProxy;
    private Proxy dynamicSidCarProxy;
    ...
    public SidCarFirstProxySelector(String localhost, RegionResolver regionResolver) {
    ...
    public Proxy next() {
        try {
            if (dynamicSidCarProxy != null) {
                return dynamicSidCarProxy;
            }
            int retry = 0;
            while (retry < 2) {
    ...
    private void reset() {
        dynamicSidCarProxy = sidCarProxy;
    ...
    public void remove(Proxy proxy) {
        try {
            synchronized (lock) {
                ...
                String region = regionResolver.resolve(proxy.getHost());
                if (Objects.equals(region, localRegion)) {
                    if (sidCarProxy == null && proxySetOtherRegion.isEmpty()) {
                        logger.warn("proxySet.size = 1, skip remove proxy! proxy = {}", proxy.toString());
                    } else {
                    ...
                    if (sidCarProxy == null && proxySetSameRegion.isEmpty()) {
                        logger.warn("proxySet.size = 1, skip remove proxy! proxy = {}", proxy.toString());
                    } else {
