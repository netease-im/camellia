...
## 快速开始一
1) 首先创建一个spring-boot的工程，然后添加以下依赖（最新1.2.3），如下：（see [sample-code](/camellia-samples/camellia-redis-proxy-samples)）:   
```
<dependency>
...
* 关于多租户的一个完整示例，具体见：[multu-telant](other/multi-telant.md)
