...
备注：1.2.8版本开始，TRANSACTION支持双写，但是要求读地址只能配置一个，且必须和写地址的第一个是一样的     
```
...
## 快速开始一
1) 首先创建一个spring-boot的工程，然后添加以下依赖（最新1.2.8），如下：（see [sample-code](/camellia-samples/camellia-redis-proxy-samples)）:   
```
<dependency>
