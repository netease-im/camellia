...
## 快速开始一
1) 首先创建一个spring-boot的工程，然后添加以下依赖（最新1.0.33），如下：（see [sample-code](/camellia-samples/camellia-redis-proxy-samples)）:   
```
<dependency>
...
下载最新版安装包并解压（v1.0.33）：
```
wget https://github.com/netease-im/camellia/releases/download/v1.0.33/camellia-redis-proxy-1.0.33.tar.gz
tar zxvf camellia-redis-proxy-1.0.33.tar.gz
cd camellia-redis-proxy-1.0.33/
```
...
java -XX:+UseG1GC -Xms2048m -Xmx2048m -server -jar camellia-redis-proxy-samples-1.0.33.jar 
```
