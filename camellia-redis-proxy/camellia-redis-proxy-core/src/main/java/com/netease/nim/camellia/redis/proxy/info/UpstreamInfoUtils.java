...
public class UpstreamInfoUtils {
    ...
    public static JSONObject monitorJson(String url) {
        ...
        switch (resource) {
            case RedisClusterResource redisClusterResource -> {
                infoJson.put("type", "cluster");
                String redisClusterInfo = getRedisClusterInfo(redisClusterResource);
                ...
                if (redisClusterInfo != null) {
                    ...
                    if (redisClusterNodeInfo != null) {
                        ...
                        for (ClusterNodeInfo clusterNodeInfo : redisClusterNodeInfo) {
                            ...
                            RedisInfo redisInfo = getRedisInfo(resource, addr);
                            if (redisInfo != null) {
            ...
            case RedisSentinelResource redisSentinelResource -> {
            ...
            case RedisResource redisResource -> {
                infoJson.put("type", "standalone");
                RedisConnectionAddr addr = new RedisConnectionAddr(redisResource.getHost(),
                        redisResource.getPort(), redisResource.getUserName(), redisResource.getPassword(), false, 0, false);
                RedisInfo redisInfo = getRedisInfo(resource, addr);
                if (redisInfo != null) {
                    infoJson.put("status", "ok");
            ...
            case null, default -> infoJsonArray.add(infoItem("type", "unknown"));
        }
    ...
    private static JSONObject toNodeJson(RedisConnectionAddr clientAddr, RedisInfo redisInfo) {
    ...
    public static String upstreamInfo(Long bid, String bgroup, IUpstreamClientTemplateFactory factory, boolean parseJson) {
        try {
                ...
                if (redisResource instanceof RedisResource) {
                    ...
                    if (redisClusterNodeInfo != null) {
                        ...
                        for (ClusterNodeInfo clusterNodeInfo : redisClusterNodeInfo) {
                            ...
                            String host = split1[0];
                            int port = Integer.parseInt(split1[1]);
                            RedisInfo redisInfo = getRedisInfo(resource, new RedisConnectionAddr(host, port, ((RedisClusterResource) redisResource).getUserName(),
    ...
    private static RedisInfo getRedisInfo(Resource resource, RedisConnectionAddr addr) {
        Map<String, String> map = getInfoMap(resource, addr.getHost(), addr.getPort(), addr.getUserName(), addr.getPassword(), new byte[][] {RedisCommand.INFO.raw()});
        if (map == null) {
            return null;
        } else {
            StringBuilder builder = new StringBuilder();
    ...
    private static String getRedisClusterInfo(RedisClusterResource redisClusterResource) {
        ...
        for (RedisClusterResource.Node node : nodes) {
            Map<String, String> map = getInfoMap(redisClusterResource, node.getHost(), node.getPort(), userName, password, new byte[][]{Utils.stringToBytes("cluster"), Utils.stringToBytes("info")});
    ...
    private static Map<String, String> getInfoMap(Resource resource, String host, int port, String userName, String password, byte[][] command) {
        RedisConnection redisConnection = null;
        try {
            redisConnection = RedisConnectionHub.getInstance().newConnection(resource, host, port, userName, password);
            if (redisConnection != null) {
                Map<String, String> map = new HashMap<>();
                CompletableFuture<Reply> future = redisConnection.sendCommand(command);
    ...
    private static boolean redisClusterSafety(List<ClusterNodeInfo> clusterNodeInfos) {
        try {
            ...
            for (ClusterNodeInfo clusterNodeInfo : clusterNodeInfos) {
                ...
                AtomicLong count = map.computeIfAbsent(masterIp, k -> new AtomicLong());
    ...
    private static List<ClusterNodeInfo> getRedisClusterNodeInfo(RedisClusterResource redisClusterResource) {
        ...
        String userName = redisClusterResource.getUserName();
        for (RedisClusterResource.Node node : nodes) {
            List<ClusterNodeInfo> clusterNodeInfos = clusterNodes(redisClusterResource, node.getHost(), node.getPort(), userName, password);
    ...
    public static List<ClusterNodeInfo> clusterNodes(Resource resource, String host, int port, String userName, String password) {
        ...
        try {
            redisConnection = RedisConnectionHub.getInstance().newConnection(resource, host, port, userName, password);
            if (redisConnection != null) {
                CompletableFuture<Reply> future = redisConnection.sendCommand(Utils.stringToBytes("cluster"), Utils.stringToBytes("nodes"));
                Reply reply = future.get(10, TimeUnit.SECONDS);
                if (reply instanceof BulkReply) {
                    String string = Utils.bytesToString(((BulkReply) reply).getRaw());
