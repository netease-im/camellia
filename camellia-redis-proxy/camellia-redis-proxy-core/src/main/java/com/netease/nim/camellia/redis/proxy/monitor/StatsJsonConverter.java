...
public class StatsJsonConverter {
    ...
    public static String converter(Stats stats) {
        ...
        for (LocalStorageExecutorStats localStorageExecutorStats : localStorageExecutorStatsList) {
            JSONObject json = new JSONObject();
            json.put("name", localStorageExecutorStats.getName());
            json.put("pending", localStorageExecutorStats.getPending());
            localStorageExecutorStatsListJsonArray.add(json);
        }
        monitorJson.put("localStorageExecutorStats", localStorageExecutorStatsListJsonArray);

        List<LocalStorageCacheStats> localStorageCacheStatsList = stats.getLocalStorageCacheStatsList();
        JSONArray localStorageCacheStatsListJsonArray = new JSONArray();
        for (LocalStorageCacheStats localStorageCacheStats : localStorageCacheStatsList) {
            JSONObject json = new JSONObject();
            json.put("operation", localStorageCacheStats.getOperation());
            json.put("rowCache", localStorageCacheStats.getRowCache());
            json.put("memTable", localStorageCacheStats.getMemTable());
            json.put("blockCache", localStorageCacheStats.getBlockCache());
            json.put("disk", localStorageCacheStats.getDisk());
            json.put("rowCacheHit", localStorageCacheStats.getRowCacheHit());
            json.put("memTableHit", localStorageCacheStats.getMemTableHit());
            json.put("blockCacheHit", localStorageCacheStats.getBlockCacheHit());
            json.put("diskHit", localStorageCacheStats.getDiskHit());
            localStorageCacheStatsListJsonArray.add(json);
        }
        monitorJson.put("localStorageCacheStats", localStorageCacheStatsListJsonArray);

        List<LocalStorageCountStats> localStorageCountStatsList = stats.getLocalStorageCountStatsList();
        JSONArray localStorageCountStatsListJsonArray = new JSONArray();
        for (LocalStorageCountStats localStorageCountStats : localStorageCountStatsList) {
            JSONObject json = new JSONObject();
            json.put("item", localStorageCountStats.getItem());
            json.put("count", localStorageCountStats.getCount());
            localStorageCountStatsListJsonArray.add(json);
        }
        monitorJson.put("localStorageCountStats", localStorageCountStatsListJsonArray);

        List<LocalStorageFileStats> localStorageFileStatsList = stats.getLocalStorageFileStatsList();
        JSONArray localStorageFileStatsListJsonArray = new JSONArray();
        for (LocalStorageFileStats localStorageFileStats : localStorageFileStatsList) {
            JSONObject json = new JSONObject();
            json.put("file", localStorageFileStats.getFile());
            json.put("readSize", localStorageFileStats.getReadSize());
            json.put("readCount", localStorageFileStats.getReadTime().getCount());
            json.put("readAvgSpendMs", localStorageFileStats.getReadTime().getAvgSpendMs());
            json.put("readMaxSpendMs", localStorageFileStats.getReadTime().getMaxSpendMs());
            json.put("readSpendMsP50", localStorageFileStats.getReadTime().getSpendMsP50());
            json.put("readSpendMsP75", localStorageFileStats.getReadTime().getSpendMsP75());
            json.put("readSpendMsP90", localStorageFileStats.getReadTime().getSpendMsP90());
            json.put("readSpendMsP95", localStorageFileStats.getReadTime().getSpendMsP95());
            json.put("readSpendMsP99", localStorageFileStats.getReadTime().getSpendMsP99());
            json.put("readSpendMsP999", localStorageFileStats.getReadTime().getSpendMsP999());

            json.put("writeSize", localStorageFileStats.getWriteSize());
            json.put("writeCount", localStorageFileStats.getWriteTime().getCount());
            json.put("writeAvgSpendMs", localStorageFileStats.getWriteTime().getAvgSpendMs());
            json.put("writeMaxSpendMs", localStorageFileStats.getWriteTime().getMaxSpendMs());
            json.put("writeSpendMsP50", localStorageFileStats.getWriteTime().getSpendMsP50());
            json.put("writeSpendMsP75", localStorageFileStats.getWriteTime().getSpendMsP75());
            json.put("writeSpendMsP90", localStorageFileStats.getWriteTime().getSpendMsP90());
            json.put("writeSpendMsP95", localStorageFileStats.getWriteTime().getSpendMsP95());
            json.put("writeSpendMsP99", localStorageFileStats.getWriteTime().getSpendMsP99());
            json.put("writeSpendMsP999", localStorageFileStats.getWriteTime().getSpendMsP999());
            localStorageFileStatsListJsonArray.add(json);
        }
        monitorJson.put("localStorageFileStats", localStorageFileStatsListJsonArray);

        List<LocalStorageTimeStats> localStorageTimeStatsList = stats.getLocalStorageTimeStatsList();
        JSONArray localStorageTimeStatsListJsonArray = new JSONArray();
        for (LocalStorageTimeStats localStorageTimeStats : localStorageTimeStatsList) {
            JSONObject json = new JSONObject();
            json.put("item", localStorageTimeStats.getItem());
            json.put("count", localStorageTimeStats.getTimeStats().getCount());
            json.put("avgSpendMs", localStorageTimeStats.getTimeStats().getAvgSpendMs());
            json.put("maxSpendMs", localStorageTimeStats.getTimeStats().getMaxSpendMs());
            json.put("spendMsP50", localStorageTimeStats.getTimeStats().getSpendMsP50());
            json.put("spendMsP75", localStorageTimeStats.getTimeStats().getSpendMsP75());
            json.put("spendMsP90", localStorageTimeStats.getTimeStats().getSpendMsP90());
            json.put("spendMsP95", localStorageTimeStats.getTimeStats().getSpendMsP95());
            json.put("spendMsP99", localStorageTimeStats.getTimeStats().getSpendMsP99());
            json.put("spendMsP999", localStorageTimeStats.getTimeStats().getSpendMsP999());
            localStorageTimeStatsListJsonArray.add(json);
        }
        monitorJson.put("localStorageTimeStats", localStorageTimeStatsListJsonArray);

        List<LocalStorageRunToCompletionStats> localStorageRunToCompletionStatsList = stats.getLocalStorageRunToCompletionStatsList();
        JSONArray localStorageRunToCompletionStatsListJsonArray = new JSONArray();
        for (LocalStorageRunToCompletionStats localStorageRunToCompletionStats : localStorageRunToCompletionStatsList) {
            JSONObject json = new JSONObject();
            json.put("command", localStorageRunToCompletionStats.getCommand());
            json.put("hit", localStorageRunToCompletionStats.getHit());
            json.put("notHit", localStorageRunToCompletionStats.getNotHit());
            json.put("hitRate", localStorageRunToCompletionStats.getHitRate());
            localStorageRunToCompletionStatsListJsonArray.add(json);
        }
        monitorJson.put("localStorageRunToCompletionStats", localStorageRunToCompletionStatsListJsonArray);

        List<LocalStorageKeyBucketStats> localStorageKeyBucketStatsList = stats.getLocalStorageKeyBucketStatsList();
        JSONArray localStorageKeyBucketStatsListJsonArray = new JSONArray();
        for (LocalStorageKeyBucketStats localStorageKeyBucketStats : localStorageKeyBucketStatsList) {
            JSONObject json = new JSONObject();
            json.put("compressType", localStorageKeyBucketStats.getCompressType());
            json.put("count", localStorageKeyBucketStats.getCount());
            json.put("keyCountAvg", localStorageKeyBucketStats.getKeyCountAvg());
            json.put("keyCountMax", localStorageKeyBucketStats.getKeyCountMax());
            json.put("compressSizeAvg", localStorageKeyBucketStats.getCompressSizeAvg());
            json.put("compressSizeMax", localStorageKeyBucketStats.getCompressSizeMax());
            json.put("decompressSizeAvg", localStorageKeyBucketStats.getDecompressSizeAvg());
            json.put("decompressSizeMax", localStorageKeyBucketStats.getDecompressSizeMax());
